...
function WaterfallChart({ steps }: { steps: ProcessingStep[] }) {
  const STEP_COLORS = ['#3b82f6','#8b5cf6','#06b6d4','#14b8a6','#6366f1','#a855f7','#0ea5e9','#2dd4bf','#4f46e5','#7c3aed'];
  const BAR_H = 32;
  const LABEL_W = 160;
  const CHART_W = 520;
  const GAP = 6;
...
export default function FileDetails({ fileId, onBack }: { fileId: string; onBack: () => void }) {
  ...
  const overallStatus: string =
    steps?.some(s => {
      ...
      return st === 'failed' || st.includes('error') || st.includes('fail');
    })
      ? 'failed'
      : steps?.length && steps.every(s => (s.status || '').toLowerCase() === 'success')
        ? 'success'
