...
import { 
  ...
  FileText, Calendar, User as UserIcon, Timer
} from 'lucide-react';
...
function WaterfallChart({ steps }: { steps: ProcessingStep[] }) {
  ...
  const bars = steps.map((s, i) => {
    ...
    cumulative += dur;
    const config = cfg(s.status);
    const isFail = config.label === 'FAILED';
    const color = isFail ? '#ef4444' : STEP_COLORS[i % STEP_COLORS.length];
    return { ...s, startPct, widthPct, dur, color, isFail, config };
  });

  const svgH = steps.length * (BAR_H + GAP) + 50;
      ...
      <div className="min-w-[850px] p-2">
        ...
        <svg width="100%" viewBox={`0 0 ${LABEL_W + CHART_W + 150} ${svgH}`} className="overflow-visible">
          ...
          {[0, 0.25, 0.5, 0.75, 1].map(pct => (
            <g key={pct}>
              <line
                x1={LABEL_W + pct * CHART_W} y1={0}
                x2={LABEL_W + pct * CHART_W} y2={svgH - 30}
          ...
          {bars.map((bar, i) => {
            ...
            const w = (bar.widthPct / 100) * CHART_W;
            ...
            return (
              ...
              >
                ...
                <rect 
                  x={x} y={y} width={w} height={BAR_H} rx={6}
                  fill={`url(#grad-${i})`}
                  className="cursor-help hover:brightness-110 transition-all shadow-sm"
                />

                {/* Status Indicator */}
                <g transform={`translate(${x + w + 8}, ${y + BAR_H / 2 - 8})`}>
                  <circle r={8} fill={bar.config.bg} cx={8} cy={8} />
                  <text x={8} y={11} textAnchor="middle" fontSize={10} fill={bar.color} className="font-bold">
                    {bar.isFail ? '!' : '✓'}
                  </text>
                </g>
      ...
      >
        ...
        <div className="px-6 py-0 flex gap-4 border-b border-border bg-muted/10">
          {(['waterfall', 'table'] as const).map(tab => (
            ...
            >
              {tab === 'waterfall' ? 'Execution View' : 'Data Explorer'}
              {activeTab === tab && (
                  ...
                  <table className="w-full text-xs">
                    <thead>
                      <tr className="bg-muted/40 border-b border-border">
                        {['#', 'Process Step', 'Status', 'Duration', 'Executed At', 'Insights'].map(h => (
                          <th key={h} className="px-6 py-4 text-left font-black text-[10px] uppercase tracking-widest text-muted-foreground">{h}</th>
                        ))}
                      </tr>
                    </thead>
                    <tbody>
                      {steps?.map((step, i) => (
                        <tr key={step.id ?? i} className="border-b border-border/30 last:border-0 hover:bg-muted/30 transition-colors group">
                          <td className="px-6 py-4 font-mono text-[10px] text-muted-foreground">{String(i + 1).padStart(2, '0')}</td>
                          <td className="px-6 py-4">
                            <span className="font-black text-foreground tracking-tight">{step.step_name}</span>
                          </td>
                          <td className="px-6 py-4"><Badge status={step.status} /></td>
                          <td className="px-6 py-4 font-mono font-bold text-muted-foreground">{fmtDur(step.duration_ms)}</td>
                          <td className="px-6 py-4 text-muted-foreground font-medium text-[10px]">{fmtDate(step.created_at)}</td>
                          <td className="px-6 py-4">
                            <div className="flex gap-2">
                              {step.output_summary && Object.keys(step.output_summary).length > 0 && (
                                <Collapsible title="Insights">
                                  <pre className="text-[10px] text-blue-300 font-mono overflow-x-auto max-h-40 scrollbar-hide">
                                    {JSON.stringify(step.output_summary, null, 2)}
                                  </pre>
                                </Collapsible>
                              )}
                              <Collapsible title="Log Trace">
                                <pre className="text-[10px] text-muted-foreground font-mono overflow-x-auto max-h-52 scrollbar-hide">
